...
lt=0
wt=0
gamemet={"Temp Run 2":4,"Hill Climb":4.5,"Angry Birds":4.5,"Subway Surfers":6,"Gesture Fit":7,"Tennis":0,"Fruit Ninja":4.5}
cgame=0
recommend=""
def run(w):
...
class page:
    ...
    def run(self):
        global age_val,height_val,weight_val,wt
        print( age_val.get(),height_val.get(),weight_val.get())
        wt=float(weight_val.get())
        print("weight :",wt,float(weight_val.get()))
...
class game:
    ...
    def launch(self):
        if self.type==0:
            proc = subprocess.Popen(['python', self.file,str(gamemet[cgame]),str(weight_val.get())], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE,start_new_session=True)
            self.pid = proc.pid
            # while True:
    ...
    def back(self):
        ft=time()-it
        global wt,cgame,gamemet
        print("Calorie Burnt :",ft/1000*gamemet[self.name]*wt*3.5/200/6000000)
        self.window.withdraw()
